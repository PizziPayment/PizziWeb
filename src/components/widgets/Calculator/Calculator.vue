<template>
  <div class="text-center">
    <v-dialog v-model="dialog" width="80vh" @click:outside="closeDialog()">
      <v-card elevation="5">
        <div id="calcWrapper">
          <Display v-bind:displayValue=dispValue v-bind:subDisplayValue=subDispValue />
          <Button
            v-for="b in buttons"
            v-bind:key="b.id"
            v-bind:bValues="b"
    ...
    computeOp: function (op) {
      if (!this.opInEffect) {
        try {
          ...
          this.prevOp = op;
          this.subDispValue =
            this.prevValue.toString() + " " + this.prevOp + "    ";
